...
import { initAudio, playChordReal, midiToFreq } from "@/lib/audio/soundfontEngine";
...
export function ChordLocksReal({ onExit }: Props) {
  ...
  const generateProblem = useCallback(() => {
    ...
    setFeedback("idle");
    setTimeout(() => void playChordReal(chord.map(midiToFreq), 1.0, "acoustic_grand_piano"), 300);
  }, [functions, level]);
  ...
  const cycleDial = (which: "func" | "mod" | "inv", dir: number) => {
    ...
    if (target) {
      ...
      if (funcInfo) void playChordReal(generateChord(60, funcInfo.type).map(midiToFreq), 0.4, "acoustic_grand_piano");
    }
  };
      ...
      {!audioReady ? (
        ...
        <>
          ...
          <Card className="bg-gradient-to-br from-amber-950/30 to-orange-950/20 border-amber-500/30 p-4 mb-4 text-center">
            ...
            <Button onClick={() => target && void playChordReal(target.chordMidis.map(midiToFreq), 1.0, "acoustic_grand_piano")} variant="outline" className="border-white/30 bg-white/10">
              ▶ Ouvir acorde
            </Button>
          </Card>
