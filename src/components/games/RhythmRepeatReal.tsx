...
import { initAudio, playNoteReal, midiToFreq } from "@/lib/audio/soundfontEngine";
...
export function RhythmRepeatReal({ onExit }: Props) {
    ...
    beats.forEach((beat, i) => {
      if (beat.active) {
        playTimeoutRef.current = setTimeout(() => {
          void playNoteReal(440, beat.duration * bpmInfo.ms / 1000 * 0.8, "acoustic_grand_piano");
          setCurrentBeat(i);
        }, totalTime);
      }
  ...
  const handleTap = useCallback(() => {
    ...
    // Toca o som do tap
    void playNoteReal(660, 0.1, "marimba");
    // Verifica se acertou
